var imgPath = '../topology/assets/icons/',//图片文件地址
    getTopology = location.origin + '/api/getTopology',//得到拓扑图
    saveTopology = location.origin + '/api/saveTopology',//保存拓扑图
...
function createCloudNode(){ //绘制核心拓扑 
    ...
    cloudNode.setImage("./assets/icons/2.png");
...
function addNode(name,x,y,cloudNode){ //将生产的单个拓扑加入到scence中 
    ...
    node.setImage("./assets/icons/1.png");
    node.text=name;
    dragable:true;
...
function dragEvt(node, canvas, text) {
    ...
    //阻止默认事件（当某被拖动的对象在另一对象容器范围内拖动时触发此事件）
    canvas.ondragover = function (e) {
        e.preventDefault();
        return false;
    };
    //创建节点（在一个拖动过程中，释放鼠标键时触发此事件）
    ...
    if(flag === 'sure') {
        ...
        if (data) {
            ...
            if (datas && datas.length == 3) {
                ...
                console.log(currentEvent.x+"-"+currentEvent.y+"-"+itemWidth+"-"+itemHeight)
